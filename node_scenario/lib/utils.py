...
def process_in_chunks(function, *args, batch_size, out=None, **kwargs):
    ...
    """
    total_size = args[0].shape[0]  # num of samples
    first_output = function(*[x[0: batch_size] for x in args])  # model(X_test[0:batch_size])
    # print(first_output[0:5])
...
def MapWordsToFloat(dataset, ids):
    data = np.copy(dataset)
    for i in ids:
        words_dict = {}
        for idx, item in enumerate(data[:,i]):
            if item not in words_dict.keys():
                words_dict[item] = len(words_dict.keys())
            data[idx, i] = words_dict[item]
        data[:,i] = data[:,i]/len(words_dict.keys())
